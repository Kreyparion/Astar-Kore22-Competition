                        ...
                        if cs < max_capacity_fleet and (cs < pre_fleet.ship_count):
                            if max_capacity_fleet != 100001:
                                if ms > nsatltas:
                                    continue
                            gravity = 0
                            v = value((r,ar,0,p,cs,ms,Ms,real_length,(new_plan,new_lmoves)),max_spawn)
...
def agent(obs: Observation, config: Configuration):
    ...
    for shipyard in shipyards:
        ...
        for (limit_step,limit_ship_nb) in incoming:
            nsatltas -= limit_ship_nb
        if action == None and kore_left >= 400 and (me_tot_ship_count-50*len(building_ships_send)>= 100*(len(shipyards)+len(building_ships_send)+50) or (me_tot_ship_count-50*len(building_ships_send) > biggest_ship_count_threat)):
            if nsatltas >= convert_cost + 10:
                pos = best_pos_shipyard(boards,shipyard.id)
                ship_number_to_send = max(convert_cost + 10, int(shipyard.ship_count/2))
                if pos != None:
